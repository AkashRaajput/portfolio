...
const groupIcons = {
  wordpress: Globe,
  nextjs: Code2,
  personal: FlaskConical,
} as const;

export function WebsiteProjectCard({ project }: { project: WebsiteProject }) {
  const Icon = groupIcons[project.group];
  const label = groupLabels[project.group];
  ...
  return (
    <motion.article variants={staggerItem} layout className="h-full">
      ...
      >
        ...
        {/* Desktop front face */}
        <div className="relative hidden h-full flex-col lg:flex lg:transition-all lg:duration-500 lg:ease-out lg:group-hover:scale-[0.97] lg:group-hover:opacity-0">
          <div className="relative aspect-[16/10] w-full overflow-hidden border-b border-border/80">
            <Image
          ...
          <div className="flex flex-1 flex-col p-6">
            <div className="flex items-start justify-between gap-3">
              <Badge className="border-primary/25 bg-background/70 text-primary backdrop-blur-sm">{label}</Badge>
              {project.featured ? (
                <Badge className="border-amber-400/30 bg-amber-400/10 text-amber-200">Featured</Badge>
              ) : null}
            </div>

            <div className="mt-5 flex size-12 items-center justify-center rounded-xl border border-border/80 bg-background/50 backdrop-blur-sm">
            ...
            <p className="mt-4 line-clamp-3 flex-1 text-sm leading-7 text-muted-foreground">{project.description}</p>
        ...
        <div className="relative flex h-full flex-col lg:hidden">
          ...
          <div className="flex flex-1 flex-col p-6">
            <div className="flex items-start justify-between gap-3">
              <Badge className="border-primary/25 bg-background/70 text-primary">{label}</Badge>
            ...
            <div className="mt-5 space-y-4">
              <ProjectDetailBlock icon={Target} label="Challenge">
                <p className="line-clamp-2">{project.challenge}</p>
              </ProjectDetailBlock>
              <ProjectDetailBlock icon={BookOpen} label="Description">
        ...
        >
          ...
          <div className="mt-4 flex-1 space-y-3.5 overflow-y-auto pr-1">
            ...
            <ProjectDetailBlock icon={CheckCircle2} label="Contributions">
            ...
            <ProjectDetailBlock icon={Sparkles} label="Business impact">
              <ul className="space-y-1.5">
